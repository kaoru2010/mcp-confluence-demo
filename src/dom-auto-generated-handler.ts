...
export class DomAutoGeneratedHandler {
  ...
  static extractAutoGeneratedSection(content: string): AutoGeneratedSection {
    try {
      ...
      const beginElement = this.findElementByText($, BEGIN_MARKER);
      const endElement = this.findElementByText($, END_MARKER);
      ...
      return this.extractContentBetweenElements($, beginElement, endElement, content);
    } catch (error) {
   ...
   */
  private static findElementByText($: cheerio.CheerioAPI, marker: string): cheerio.Cheerio<any> | null {
  ...
  private static extractContentBetweenElements(
    ...
    if (!foundContent) {
      ...
      contentParts = this.extractContentByTraversal($, beginElement, endElement);
    }

    const before = this.getContentBefore($, beginElement, originalContent);
    const after = this.getContentAfter($, endElement, originalContent);
  ...
  private static extractContentByTraversal(
    ...
    $("*").each((_, element) => {
      ...
      if (capturing) {
        ...
        if (htmlContent && htmlContent.trim() && !this.isEmptyParagraph(htmlContent)) {
          contentParts.push(htmlContent);
        }
      }
    });
  ...
  static replaceAutoGeneratedSection(
    ...
    const section = this.extractAutoGeneratedSection(originalContent);
    ...
    // マーカー間の内容を置き換え
    return this.replaceContentBetweenMarkers(originalContent, newGeneratedContent);
  }
  ...
  static getAutoGeneratedContent(content: string): string {
    const section = this.extractAutoGeneratedSection(content);
