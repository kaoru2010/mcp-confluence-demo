...
program
  ...
  .action(async (url: string, newContent: string, options) => {
    try {
      ...
      console.log(`Successfully updated page. New version: ${updatedPage.version.number}`);
...
program
  ...
  .action(async (url: string, options) => {
    try {
      ...
      console.log(`  Has auto-generated markers: ${DomAutoGeneratedHandler.hasAutoGeneratedMarkers(page.body.storage.value)}`);
    } catch (error) {
      console.error("Error:", error instanceof Error ? error.message : error);
      process.exit(1);
    }
