...
export class XmlAutoGeneratedHandler {
   ...
   */
  static async extractAutoGeneratedSection(
    content: string,
  ): Promise<AutoGeneratedSection> {
    try {
      // まず文字列ベースで簡単にチェック
      ...
      // XMLとしてパース可能かチェック
      try {
        await XmlAutoGeneratedHandler.parser.parseStringPromise(
          `<root>${content}</root>`,
        );
        return await XmlAutoGeneratedHandler.extractFromXml(content);
      } catch (xmlError) {
        // XMLパースが失敗した場合は文字列ベースで処理
        console.warn(
          "XML parsing failed, falling back to string-based processing:",
          xmlError,
        );
        return XmlAutoGeneratedHandler.extractFromString(content);
      }
    } catch (error) {
   ...
   * XMLベースの抽出
   */
  private static async extractFromXml(
    content: string,
  ): Promise<AutoGeneratedSection> {
    const wrappedContent = `<root>${content}</root>`;
    const parsed =
      await XmlAutoGeneratedHandler.parser.parseStringPromise(wrappedContent);

    // XML構造を文字列に戻してマーカーを探す
    const xmlString = XmlAutoGeneratedHandler.builder.buildObject(parsed);
    const cleanContent = xmlString.replace(/^<root>|<\/root>$/g, "");

    return XmlAutoGeneratedHandler.extractFromString(cleanContent);
  }
  ...
  static async replaceAutoGeneratedSection(
    ...
    const section =
      await XmlAutoGeneratedHandler.extractAutoGeneratedSection(
        originalContent,
      );

    if (!section.found) {
      // マーカーが見つからない場合は、適切なXML構造で追加
      return XmlAutoGeneratedHandler.appendAutoGeneratedSection(
        originalContent,
        newGeneratedContent,
      );
    }
    ...
    // マーカー間の内容を置き換え（構造を保持）
    return XmlAutoGeneratedHandler.replaceWithProperStructure(
      section,
      newGeneratedContent,
    );
  }
...
`;
  ...
  static async getAutoGeneratedContent(content: string): Promise<string> {
    const section =
      await XmlAutoGeneratedHandler.extractAutoGeneratedSection(content);
